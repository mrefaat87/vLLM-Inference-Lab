"""
Test 30A: FP8 KV Cache — Latency Invariance
...
Compare results against stage2_test29a_results.json (T4 E5M2) (FP16 baseline).

Usage: python3 stage2_test30a_fp8_latency.py --host http://<ip>:8000
"""
...
def main():
    global HOST
    parser = argparse.ArgumentParser(description="Test 30A: FP8 KV Cache — Latency Invariance")
    parser.add_argument("--host", required=True)
    parser.add_argument("--duration", type=int, default=60, help="Seconds per RPS level")
    args = parser.parse_args()
    HOST = args.host.rstrip("/")

    print(f"Test 30A: FP8 KV Cache — Latency Invariance")
    print(f"Target: {HOST} | Model: {MODEL}")
    print(f"KV Cache: fp8_e4m3 (A10G Ampere) (compare against Test 29A T4 fp8_e4m3 (A10G Ampere) + Test 25 fp16 baseline)")
    ...
    # Save
    output_file = "stage2_test30a_results.json"
