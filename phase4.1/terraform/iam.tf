...
# ─── Variant H — temp inline policy for FSx mount lookup ──────────────────────
# The Lustre client on the worker node calls fsx:DescribeFileSystems to resolve
# the mount metadata before opening the LNet connection. Without this, mount
# hangs silently. Scoped READ-ONLY: the node never creates/deletes filesystems.
# Detached in Stage 9 of the Variant H plan to keep node permissions tight.
resource "aws_iam_role_policy" "karpenter_node_fsx_describe" {
  name = "fsx-describe-temp"
  role = aws_iam_role.karpenter_node.name
  ...
  oidc_providers = {
    main = {
      ...
      namespace_service_accounts = [
        "inference-system:vllm-worker",  # legacy from phase4 platform
        "baseline:baseline-runner",      # phase4.1 baseline-pod (this phase)
      ]
    }
  }
}
