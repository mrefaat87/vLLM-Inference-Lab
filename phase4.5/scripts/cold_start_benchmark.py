...
Usage:
    ...
    python3 phase4.5/tests/cold_start_benchmark.py --host http://localhost:8080

    # Subset of configs
    python3 phase4.5/tests/cold_start_benchmark.py --host http://localhost:8080 --configs naked,+s3-init,+startup-probe

    # Skip sleep test
    python3 phase4.5/tests/cold_start_benchmark.py --host http://localhost:8080 --skip-sleep

    # Dry run (print YAMLs without applying)
    python3 phase4.5/tests/cold_start_benchmark.py --dry-run
"""
...
def verify_phase4_context() -> str:
    """Verify kubectl context contains 'inference-phase4-5'. Hard exit if wrong.
    ...
    if "inference-phase4-5" not in context:
        print(f"\n  ABORT: Wrong kubectl context '{context}'.")
        print(f"  Expected context containing 'inference-phase4-5'.")
        print(f"  Switch with: aws eks update-kubeconfig --name inference-phase4-5 --region us-east-1")
        sys.exit(1)
...
spec:
  amiFamily: AL2
{ami_selector}
  role: "inference-phase4-5-karpenter-node"

  subnetSelectorTerms:
    - tags:
        karpenter.sh/discovery: "inference-phase4-5"
...
def preflight(configs_to_run: list) -> None:
    ...
    if needs_prebaked:
        ...
        if ami_id and ami_id != "None":
            ...
            print("         Build with: bash phase4.5/scripts/build-ami.sh")
    ...
    if needs_soci:
        ...
        if ami_id and ami_id != "None":
            ...
            print("         Build with: bash phase4.5/scripts/build-ami-soci.sh")
