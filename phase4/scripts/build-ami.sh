...
#
# USAGE:
#   bash phase4/scripts/build-ami.sh
#   bash phase4/scripts/build-ami.sh --region us-west-2
#
# PREREQUISITES:
...
if [ "${EXISTING_AMI}" != "None" ] && [ "${EXISTING_AMI}" != "null" ] && [ -n "${EXISTING_AMI}" ]; then
    ...
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        ...
        echo ""
        echo "To use this AMI in Karpenter, update phase4/k8s/karpenter/gpu-nodepool.yaml:"
