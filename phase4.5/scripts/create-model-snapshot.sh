...
#
# USAGE:
#   bash phase4.5/scripts/create-model-snapshot.sh
#   bash phase4.5/scripts/create-model-snapshot.sh --region us-west-2
#   bash phase4.5/scripts/create-model-snapshot.sh --fsr   # enable Fast Snapshot Restore
#
# PREREQUISITES:
#   - AWS CLI configured with credentials
#   - Model already uploaded to S3 (run phase4.5/scripts/upload-model-to-s3.sh first)
...
if ! aws s3 ls "s3://${BUCKET}/${MODEL_PATH}/" --region "${REGION}" &>/dev/null; then
    ...
    echo "  bash phase4.5/scripts/upload-model-to-s3.sh"
    exit 1
fi
...
if [ "${ENABLE_FSR}" = true ]; then
    ...
    echo "  bash phase4.5/scripts/create-model-snapshot.sh --fsr"
    echo "  # or manually:"
