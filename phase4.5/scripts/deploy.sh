...
#
# USAGE:
#   bash phase4.5/scripts/deploy.sh                # deploy with Policy A (queue-only)
#   bash phase4.5/scripts/deploy.sh policy-b       # deploy with Policy B (queue-eager)
#   bash phase4.5/scripts/deploy.sh policy-c       # deploy with Policy C (composite-conservative)
#   bash phase4.5/scripts/deploy.sh policy-d       # deploy with Policy D (composite-aggressive)
...
case "${POLICY}" in
    policy-a)
        KEDA_FILE="phase4.5/k8s/autoscaling/policy-a-queue-only.yaml"
        POLICY_DESC="Policy A: Queue-only (value=5)"
        ;;
    policy-b)
        KEDA_FILE="phase4.5/k8s/autoscaling/policy-b-queue-eager.yaml"
        POLICY_DESC="Policy B: Queue-eager (value=3)"
        ;;
    policy-c)
        KEDA_FILE="phase4.5/k8s/autoscaling/policy-c-composite-conservative.yaml"
        POLICY_DESC="Policy C: Composite-conservative (queue + KV cache)"
        ;;
    policy-d)
        KEDA_FILE="phase4.5/k8s/autoscaling/policy-d-composite-aggressive.yaml"
...
kubectl apply -f phase4.5/k8s/scaling-config/configmap.yaml
...
kubectl apply -f phase4.5/k8s/karpenter/gpu-nodepool.yaml
...
kubectl apply -f phase4.5/k8s/worker/pdb.yaml
...
DASHBOARD_FILE="phase4.5/k8s/monitoring/grafana-dashboard-patch.yaml"
...
kubectl apply -f phase4.5/k8s/worker/deployment-patch.yaml
...
echo "    2. Switch policy:     bash phase4.5/scripts/deploy.sh policy-b"
echo "    3. Run policy test:   python3 phase4.5/tests/scaling_policy_comparison.py --host http://localhost:8080"
echo "    4. Rollback:          bash phase4.5/scripts/rollback.sh"
echo ""
echo "  Prerequisites (one-time, if not already done):"
echo "    - Build SOCI AMI:     bash phase4.5/scripts/build-ami-soci.sh (or build-ami.sh without SOCI)"
echo "    - Setup SOCI:         bash phase4.5/scripts/setup-soci.sh (push vLLM image + index to ECR)"
echo "    - Upload model to S3: bash phase4.5/scripts/upload-model-to-s3.sh"
echo ""
