...
## Group I: Infrastructure Optimization (Phase 6+)

Items deferred from earlier phases that only become valuable once the system has the right shape to use them.

### I-1. Revisit SOCI lazy snapshotter (deferred from Phase 4.1)

**Why deferred:** Phase 4.1 compared three image-pull strategies (Docker Hub vs ECR same-region vs pre-baked AMI) on a single vLLM image. Pre-baked AMI dominates for one image: zero runtime risk, zero registry dependency at scale-out time, predictable. SOCI's value (lazy-fetch container layers via HTTP range requests instead of full pull) is only a real win when N images make pre-baking impractical — and Phase 4.1 has N=1.

**Why Phase 6:** Phase 6 introduces multi-model serving and graceful degradation (bin-packing, tiered fallback). That means many images on every node — not just vLLM-stock, but model-specific variants, fallback engines (TGI/SGLang), maybe specialized quantization builds. Pre-baking all of them into one AMI explodes AMI size and rebuild churn. SOCI flips the cost: each new image is a cheap index push to ECR, and only the bytes a container actually touches get fetched.

**Setup already scaffolded** (don't rebuild from scratch when you get here):
- `phase4.1/packer/gpu-node-soci.pkr.hcl` — SOCI-enabled AMI variant with soci-snapshotter systemd service + containerd proxy snapshotter config.
- `phase4.1/scripts/setup-soci.sh` — pushes image + SOCI index to ECR (SOCI requires ECR for OCI referrers API support; Docker Hub doesn't expose referrers).

**Known gotcha** (per Phase 4.1 exploration): older soci-snapshotter versions had a "duplicate image config" bug when the snapshotter was enabled but the index was missing or malformed — containerd would fall back to a normal pull *and* the SOCI plugin would also try to handle it. Pin a recent version (v0.7+) and ensure `discard_unpacked_layers = false` in containerd config.

**Realistic expectation for vLLM specifically:** SOCI shines for stateless web apps with small working sets. vLLM at startup imports a deep dependency tree (torch, transformers, CUDA runtime, flash-attn) — working set is probably 2-3GB out of 9.5GB, not 500MB. Plus model weights load competes with SOCI's background fetches. Don't expect 10x speedup; expect maybe 2-3x faster time-to-container-running than a full network pull, but TTFT-ready may improve less because imports still need to happen.

**Measurement approach when revisiting:** repeat the Phase 4.1 3-way comparison harness (`run_pull_comparison.py`) with SOCI as a 4th or 5th variant. Hold the same hardware fingerprint, same model weights, same image bytes. The lift over pure pre-baked AMI is the question — if it's negative, pre-baking still wins for that specific image and SOCI is reserved for the high-N case.