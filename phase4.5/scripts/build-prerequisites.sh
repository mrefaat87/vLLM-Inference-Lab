...
#
# SAFETY:
#   - Only touches inference-phase4-5 resources (ECR, AMIs, EBS snapshots)
...
#
# USAGE:
#   bash phase4.5/scripts/build-prerequisites.sh
#   bash phase4.5/scripts/build-prerequisites.sh --skip-ami      # skip pre-baked AMI
#   bash phase4.5/scripts/build-prerequisites.sh --skip-soci-ami # skip SOCI AMI
#   bash phase4.5/scripts/build-prerequisites.sh --only-ec2      # only run SOCI+streamer on EC2
#
# PREREQUISITES:
#   - AWS CLI configured
...
BUILDER_IAM_PROFILE="inference-phase4-5-karpenter-node"  # has ECR push permissions
...
for arg in "$@"; do
    case $arg in
        ...
        --help|-h)
            echo "Usage: bash phase4.5/scripts/build-prerequisites.sh [OPTIONS]"
...
if ! aws iam get-instance-profile --instance-profile-name "${BUILDER_IAM_PROFILE}" &>/dev/null; then
    ...
    error "Expected: inference-phase4-5-karpenter-node (from Phase 4 Terraform)."
    exit 1
fi
...
SUBNET_ID=$(aws ec2 describe-subnets \
    --filters "Name=tag:karpenter.sh/discovery,Values=inference-phase4-5" \
    --query 'Subnets[0].SubnetId' \
    --output text \
    --region "${REGION}")

if [ -z "${SUBNET_ID}" ] || [ "${SUBNET_ID}" == "None" ]; then
    error "No subnet found with tag karpenter.sh/discovery=inference-phase4-5"
...
aws ssm put-parameter \
    --name "/inference-phase4-5/prereq-build-status" \
...
if [ ${ELAPSED} -ge ${MAX_WAIT} ]; then
    ...
    echo "  aws ssm get-parameter --name /inference-phase4-5/prereq-build-status --region ${REGION}"
