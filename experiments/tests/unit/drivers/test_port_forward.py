...
@patch("experiments.drivers.k8s_common.time.sleep", lambda _: None)  # don't wait 2s
@patch("experiments.drivers.k8s_common.subprocess.Popen")
def test_start_spawns_port_forward_by_default(popen_mock: MagicMock) -> None:
    proc = MagicMock()
...
@patch("experiments.drivers.k8s_common.subprocess.Popen")
def test_stop_terminates_port_forward(popen_mock: MagicMock) -> None:
    proc = MagicMock()
    proc.wait.return_value = 0
    popen_mock.return_value = proc
...
@patch("experiments.drivers.k8s_common.time.sleep", lambda _: None)
@patch("experiments.drivers.k8s_common.subprocess.Popen")
def test_stop_force_kills_unresponsive_port_forward(popen_mock: MagicMock) -> None:
    proc = MagicMock()
...
@patch("experiments.drivers.k8s_common.time.sleep", lambda _: None)
@patch("experiments.drivers.k8s_common.subprocess.Popen")
def test_endpoint_url_points_at_localhost_under_port_forward(popen_mock: MagicMock) -> None:
    proc = MagicMock()
    popen_mock.return_value = proc
