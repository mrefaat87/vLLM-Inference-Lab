"""
Test 25: Tail Latency Under Sustained Load (P50 vs P99)

Open-loop load generation: sends requests at fixed RPS rates regardless of
whether previous requests have completed. This simulates real traffic patterns
where users don't wait for the server to be "ready".

Think of it like target tracking scaling: at low RPS, P50 ≈ P99 (all requests
are fast). As RPS approaches capacity, P99 diverges dramatically from P50 —
this is the "tail" that violates SLAs.

Usage: python3 stage2_test25_tail_latency.py --host http://<ip>:8000
"""
...
RPS_LEVELS = [1, 2, 4, 8, 12, 16, 20]
DURATION_SEC = 60  # Sustain each rate for 60 seconds

# Standard prompt — short enough to not dominate latency
...
def main():
    global HOST
    parser = argparse.ArgumentParser(description="Test 25: Tail Latency Under Sustained Load")
    parser.add_argument("--host", required=True)
    parser.add_argument("--duration", type=int, default=60, help="Seconds per RPS level")
    args = parser.parse_args()
    HOST = args.host.rstrip("/")

    print(f"Test 25: Tail Latency Under Sustained Load")
    ...
    # Find the "SLA-safe" RPS
    for r in all_results:
        if "ttft_p99" in r and r["ttft_p99"] > 1.0:
            print(f"\n  ⚠ TTFT P99 exceeds 1s at {r['rps']} RPS — "
                  f"SLA-safe capacity is {all_results[all_results.index(r) - 1]['rps'] if all_results.index(r) > 0 else 0} RPS")
            break

    # Save
    output_file = "/Users/mrefaat/Apps/LLM_Hands_On/vLLM_Inference/stage2_test25_results.json"
