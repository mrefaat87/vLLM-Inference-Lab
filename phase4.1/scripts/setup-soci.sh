...
#
# USAGE:
#   bash phase4/scripts/setup-soci.sh
#
# AFTER RUNNING THIS:
#   1. Build the SOCI-enabled AMI: bash phase4/scripts/build-ami-soci.sh
#   2. Update gpu-nodepool.yaml amiSelectorTerms tag if using a new AMI name
#   3. Deploy: bash phase4/scripts/deploy.sh policy-d
#
set -euo pipefail
...
echo "     bash phase4/packer/build-ami-soci.sh"
echo "  2. Deploy with the new AMI:"
echo "     bash phase4/scripts/deploy.sh policy-d"
echo ""
